...
export const CreateSelect = ({ multiple, value, options, onChange, onCreateOption }: selcetProps) => {
    ...
    return (
        <div tabIndex={0} className="w-96 p-2 min-h-[3.5em] border-2 border-zinc-400 flex items-center gap-5 relative rounded-md cursor-text justify-between"
            ...
            <ul tabIndex={0} style={{ top: "calc(100% + .30em)" }} className={`absolute border-2 border-zinc-400 rounded-md w-full left-0 shadow-2xl max-h-[15em] overflow-y-auto bg-white ${isOpen ? "visible" : "hidden"}`}
            ...
            >
                {filteredOptions.length > 0 ? (
                    ...
                    (<p className="hover:bg-zinc-200 p-2 cursor-pointer"
                        onClick={() => { onCreateOption(inputValue); setInptValue("") }}
                    >
                        {inputValue.length > 0 ? "Criar nova tag " + '"' + inputValue + '"' + "." : "Digite para criar uma nova tag."}
                    </p>)
                }
            </ul>
        </div >
